...
public class HostFamilyBackground implements Serializable {
   ...
   @Column(length = 100) private String account;
   ...
   @Temporal(TemporalType.TIMESTAMP) private Date dateOrderClosed;

   @Temporal(TemporalType.TIMESTAMP) private Date dateOrderReceived;
   ...
   @Column(nullable = false, length = 100) private String resultStatus;

   @Column(length = 200) private String resultURL;
   ...
   @ManyToOne @JoinColumn(name = "hostFamilyGoId") private HostFamily hostFamily;

   // bi-directional many-to-one association to HostFamilyMember
   @ManyToOne @JoinColumn(name = "hostFamilyMemberId") private HostFamilyMember hostFamilyMember;
