...
public class SubPartnerInterfaceImpl implements SubPartnerInterface {
	...
	public WSDefaultResponse createSubPartnerDetail(com.ccighgo.service.transport.partner.beans.subpartnerdetail.SubPartnerDetail subPartner) {
		...
		try {
			...
			responce.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO, ErrorCode.SUB_PARTNER_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_SUCCESS)));
		} catch (CcighgoException e) {
	...
	public AllSalutations getAllSalutation() {
		...
		try {
			...
			if (as.getSalutationList() == null && as.getSalutationList().isEmpty())
				as.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO, ErrorCode.FETCH_SALUTATION.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_SUCCESS)));
			else
				as.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO, ErrorCode.FETCH_SALUTATION.getValue(), messageUtil.getMessage(CCIConstants.NO_RECORD)));
		} catch (Exception e) {
			ExceptionUtil.logException(e, LOGGER);
