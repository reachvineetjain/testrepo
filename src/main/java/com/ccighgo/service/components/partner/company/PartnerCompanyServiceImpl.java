...
public class PartnerCompanyServiceImpl implements PartnerCompanyService {
	...
	public PartnerCompanyDetail getPartnerCompanyDetails(String partnerGoId) {
		...
		try {
			...
			// if (partner.getPartnerGoId() ==
			// contact.getPartner().getPartnerGoId()) {
		...
		try {
			...
			if (partner != null) {
				...
				if (!(partnerLogin.getLoginName().equals(partnerCompanyDetail.getPartnerCompanyDetails().getUserName()))) {
					...
					pUser.setRecieveNotificationEmails(partnerCompanyDetail.getPartnerPrimaryContact().isPrimaryContactShouldRecieveCCINotification() ? CCIConstants.ACTIVE : CCIConstants.INACTIVE);
				...
				updatedObject.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO, ErrorCode.NO_RECORD.getValue(), messageUtil.getMessage(CCIConstants.NO_RECORD)));
	...
	public Response deletePartnerOffice(String partnerOfficeId) {
		...
		if (partnerOfficeId == null || Integer.valueOf(partnerOfficeId) == 0 || Integer.valueOf(partnerOfficeId) < 0 || partnerOfficeId == null) {
			...
			try {
				...
				/*
				 * As per Tushad and Phani, 1. Check if the office getting
				 * deleted is partner primary office: --If yes return message to
				 * set another office as primary and move all associated users
				 * to the new office 2. If office is not primary office but
				 * users are associated with the office: --Return message saying
				 * move users to some other office and then delete
				 */
				// get the partner
