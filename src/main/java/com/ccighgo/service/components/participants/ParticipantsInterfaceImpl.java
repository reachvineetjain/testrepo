...
public class ParticipantsInterfaceImpl implements ParticipantsInterface {
		...
		try {
			...
			if (participants != null) {
				for (Participant participant : participants) {
					...
					participantProgramOption.setParticipantProgramOptionId(
							participant.getDepartmentProgramOption().getDepartmentProgramOptionId());
					participantProgramOption.setParticipantProgramOption(
							participant.getDepartmentProgramOption().getProgramOptionName());
					...
					participantSeason
							.setParticipantSeasonProgramId(participant.getDepartmentProgram().getDepartmentProgramId());
				...
				participantsActiveList.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO,
						ErrorCode.DEFAULT_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_SUCCESS)));
			}
		} catch (Exception e) {
			participantsActiveList.setStatus(componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR,
		...
		try {
			...
			for (Participant participant : leadParticipants) {
				...
				participantProgramOption
						.setParticipantProgramOption(participant.getDepartmentProgramOption().getProgramOptionName());
			...
			participantsLeadList.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO,
					ErrorCode.DEFAULT_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_SUCCESS)));

		} catch (Exception e) {
			participantsLeadList.setStatus(componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR,
					ErrorCode.DEFAULT_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_FAILURE)));
			ExceptionUtil.logException(e, logger);
		}
		...
		try {
			if (newManualParticipant != null) {
				for (AddNewManualParticipant p : newManualParticipant.getDetails()) {
					...
					com.ccighgo.db.entities.UserType ParticipantUserType = userTypeRepository
							.findOne(CCIConstants.PARTICIPANT_USER_TYPE);
					...
					try {
						ParticipantStatus participantStatus = participantStatusRepository
								.findOne(CCIConstants.PARTICIPANT_STATUS_PENDING_VERIFICATION);
						...
						DepartmentProgramOption departmentProgramOption = departmentProgramOptions
								.findOne(p.getDepartmentProgramOptionId());
						...
						/**
						 * According to phani last discussion regarding
						 * start,end dates check with department program and
						 * wich department program option and get the values
						 * from departmentProgram table
						 */
						switch (departmentProgram.getProgramName()) {
						case "J-1HS":
							SeasonJ1Detail j1 = seasonJ1DetailsRepository.findJ1DetailsBySeasonId(p.getSeasonId());
							switch (departmentProgramOption.getProgramOptionCode()) {
							case "Aug-FY":
								participant.setStartDate(j1.getAugFullYearStartDate());
								participant.setEndDate(j1.getAugFullYearEndDate());

								if (j1.getAugFullYearStartDate() == null && j1.getAugFullYearEndDate() == null) {
									participant.setStartDate(j1.getFirstSemStartDate());
									participant.setEndDate(j1.getFirstSemEndDate());
								}
								break;
							case "Aug-1Sem":
								participant.setStartDate(j1.getFirstSemStartDate());
								...
								break;
							case "Jan-FY":
								participant.setStartDate(j1.getJanFullYearStartDate());
								participant.setEndDate(j1.getJanFullYearEndDate());

								if (j1.getJanFullYearStartDate() == null && j1.getJanFullYearEndDate() == null) {
									participant.setStartDate(j1.getSecondSemStartDate());
									participant.setEndDate(j1.getSecondSemEndDate());
								}
								break;
							case "Jan-2Sem":
								participant.setStartDate(j1.getSecondSemStartDate());
							...
							break;
						case "F-1":
							SeasonF1Detail f1 = seasonF1DetailsRepository.getAllSeasonF1DetailById(p.getSeasonId());
							switch (departmentProgramOption.getProgramOptionCode()) {
							case "Aug-FY":
								participant.setStartDate(f1.getAugFullYearStartDate());
								participant.setEndDate(f1.getAugFullYearEndDate());

								if (f1.getAugFullYearStartDate() == null && f1.getAugFullYearEndDate() == null) {
									participant.setStartDate(f1.getFirstSemStartDate());
									participant.setEndDate(f1.getFirstSemEndDate());
								}
								break;
							case "Aug-1Sem":
								participant.setStartDate(f1.getFirstSemStartDate());
								...
								break;
							case "Jan-FY":
								participant.setStartDate(f1.getJanFullYearStartDate());
								participant.setEndDate(f1.getJanFullYearEndDate());

								if (f1.getJanFullYearStartDate() == null && f1.getJanFullYearEndDate() == null) {
									participant.setStartDate(f1.getSecondSemStartDate());
									participant.setEndDate(f1.getSecondSemEndDate());
								}
								break;
							case "Jan-2Sem":
								participant.setStartDate(f1.getSecondSemStartDate());
							...
							break;
						case "STP-IHP":
							SeasonIHPDetail ihp = seasonIHPDetailRepository.findIHPDetailsBySeasonId(p.getSeasonId());

							participant.setStartDate(ihp.getStartDate());
							participant.setEndDate(ihp.getEndDate());
							break;
						...
						// if (season != null && partner != null) {
						// PartnerSeason ps =
						// partnerSeasonsRepository.findPartnerSeasonBySeasonIdProgramIdPartnerGoId(season.getSeasonId(),
						// p.getDepartmentId(), partner
						// .getGoIdSequence().getGoId());
						//
						// if (ps != null) {
						// participant.setStartDate(ps.getPartnerSeasonStartDate());
						...
						ExceptionUtil.logException(e, logger);
						newManualParticipant.setStatus(componentUtils.getStatus(CCIConstants.FAILURE,
								CCIConstants.TYPE_ERROR, ErrorCode.DEFAULT_CODE.getValue(),
			...
			newManualParticipant.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO,
					ErrorCode.DEFAULT_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_SUCCESS)));
		} catch (Exception e) {
			newManualParticipant.setStatus(componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR,
					ErrorCode.DEFAULT_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_FAILURE)));
			ExceptionUtil.logException(e, logger);
	...
	public NewManualParticipant editNewParticipant(Integer participantId) {
		...
		try {
			...
			editedParticipant.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO,
					ErrorCode.DEFAULT_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_SUCCESS)));
		} catch (Exception e) {
			editedParticipant.setStatus(componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR,
					ErrorCode.DEFAULT_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_FAILURE)));
			ExceptionUtil.logException(e, logger);
		}
	...
	public ProgramOptionsForParticipants getAllAvailableProgramOptions(int partnerId, int seasonId,
			int departmentProgramId) {
		ProgramOptionsForParticipants programOptionsForParticipants = new ProgramOptionsForParticipants();
		try {
			List<DepartmentProgramOption> options = departmentProgramOptions
					.findProgramOptionsByDepartmentProgramId(departmentProgramId);
			...
			programOptionsForParticipants
					.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO,
							ErrorCode.DEFAULT_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_SUCCESS)));
		} catch (Exception e) {
			programOptionsForParticipants
					.setStatus(componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR,
							ErrorCode.DEFAULT_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_FAILURE)));
			ExceptionUtil.logException(e, logger);
		}
	...
	public ProgramOptionsForParticipants getAllAvailableProgramOptions(int partnerId, int seasonId) {
		ProgramOptionsForParticipants programOptionsForParticipants = new ProgramOptionsForParticipants();
		try {
			List<PartnerSeason> partnerSeasons = partnerSeasonsRepository
					.findPartnerSeasonByPartnerGoIdAndSeasonId(partnerId, seasonId);
			...
			if (partnerSeasons != null) {
				for (PartnerSeason partnerSeason : partnerSeasons) {
					...
					List<DepartmentProgramOption> options = departmentProgramOptions
							.findProgramOptionsByDepartmentProgramId(departmentProgram.getDepartmentProgramId());
					for (DepartmentProgramOption o : options) {
						ProgramOptionsForParticipantsDetails details = new ProgramOptionsForParticipantsDetails();
			...
			if (participants != null) {
				for (Participant participant : participants) {
					...
					if (participant.getParticipantStatus() != null) {
						...
						if (participant.getParticipantStatus().getIsApplicationStatus() == CCIConstants.ACTIVE) {
							details.setParticipantApplicationStatus(
									participant.getParticipantStatus().getParticipantStatusName());
							details.setParticipantApplicationStatusId(
					...
					if (participant.getDepartmentProgramOption() != null) {
						details.setParticipantProgramOptionId(
								participant.getDepartmentProgramOption().getDepartmentProgramOptionId());
						details.setParticipantProgramOption(
								participant.getDepartmentProgramOption().getProgramOptionName());
					}
					...
					try {
						...
						if (participant.getSeason().getSeasonF1details() != null
								&& programName.equalsIgnoreCase(CCIConstants.HSP_F1)) {
							if (participant.getSeason().getSeasonF1details() != null
									&& !participant.getSeason().getSeasonF1details().isEmpty())
								details.setParticipantSeasonName(
										participant.getSeason().getSeasonF1details().get(0).getProgramName());
						} else if (participant.getSeason().getSeasonJ1details() != null
								&& programName.equalsIgnoreCase(CCIConstants.HSP_J1_HS)) {
							if (participant.getSeason().getSeasonJ1details() != null
									&& !participant.getSeason().getSeasonJ1details().isEmpty())
								details.setParticipantSeasonName(
										participant.getSeason().getSeasonJ1details().get(0).getProgramName());
						} else if (participant.getSeason().getSeasonCapdetails() != null
								&& programName.equalsIgnoreCase(CCIConstants.WP_WT_CAP)) {
							if (participant.getSeason().getSeasonCapdetails() != null
									&& !participant.getSeason().getSeasonCapdetails().isEmpty())
								details.setParticipantSeasonName(
										participant.getSeason().getSeasonCapdetails().get(0).getProgramName());
						} else if (participant.getSeason().getSeasonWnTsummerDetails() != null
								&& programName.equalsIgnoreCase(CCIConstants.WP_WT_SUMMER)) {
							if (participant.getSeason().getSeasonWnTsummerDetails() != null
									&& !participant.getSeason().getSeasonWnTsummerDetails().isEmpty())
								details.setParticipantSeasonName(
										participant.getSeason().getSeasonWnTsummerDetails().get(0).getProgramName());
						} else if (participant.getSeason().getSeasonWnTwinterDetails() != null
								&& programName.equalsIgnoreCase(CCIConstants.WP_WT_WINTER)) {
							if (participant.getSeason().getSeasonWnTwinterDetails() != null
									&& !participant.getSeason().getSeasonWnTwinterDetails().isEmpty())
								details.setParticipantSeasonName(
										participant.getSeason().getSeasonWnTwinterDetails().get(0).getProgramName());
						} else if (participant.getSeason().getSeasonWnTspringDetails() != null
								&& programName.equalsIgnoreCase(CCIConstants.WP_WT_SPRING)) {
							if (participant.getSeason().getSeasonWnTspringDetails() != null
									&& !participant.getSeason().getSeasonWnTspringDetails().isEmpty())
								details.setParticipantSeasonName(
										participant.getSeason().getSeasonWnTspringDetails().get(0).getProgramName());
						} else if (participant.getSeason().getSeasonHsadetails() != null
								&& programName.equalsIgnoreCase(CCIConstants.GHT_HS_ABRD)) {
							if (participant.getSeason().getSeasonHsadetails() != null
									&& !participant.getSeason().getSeasonHsadetails().isEmpty())
								details.setParticipantSeasonName(
										participant.getSeason().getSeasonHsadetails().get(0).getProgramName());
						} else if (participant.getSeason().getSeasonWadetails() != null
								&& programName.equalsIgnoreCase(CCIConstants.GHT_WRK_ABRD)) {
							if (participant.getSeason().getSeasonWadetails() != null
									&& !participant.getSeason().getSeasonWadetails().isEmpty())
								details.setParticipantSeasonName(
										participant.getSeason().getSeasonWadetails().get(0).getProgramName());

						} else if (participant.getSeason().getSeasonTadetails() != null
								&& programName.equalsIgnoreCase(CCIConstants.GHT_TEACH_ABRD)) {
							if (participant.getSeason().getSeasonTadetails() != null
									&& !participant.getSeason().getSeasonTadetails().isEmpty())
								details.setParticipantSeasonName(
										participant.getSeason().getSeasonTadetails().get(0).getProgramName());

						} else if (participant.getSeason().getSeasonLsdetails() != null
								&& programName.equalsIgnoreCase(CCIConstants.GHT_LANG_SCL)) {
							if (participant.getSeason().getSeasonLsdetails() != null
									&& !participant.getSeason().getSeasonLsdetails().isEmpty())
								details.setParticipantSeasonName(
										participant.getSeason().getSeasonLsdetails().get(0).getProgramName());

						} else if (participant.getSeason().getSeasonVadetails() != null
								&& programName.equalsIgnoreCase(CCIConstants.GHT_VOL_ABRD)) {
							if (participant.getSeason().getSeasonVadetails() != null
									&& !participant.getSeason().getSeasonVadetails().isEmpty())
								details.setParticipantSeasonName(
										participant.getSeason().getSeasonVadetails().get(0).getProgramName());

						} else if (participant.getSeason().getSeasonIhpdetails() != null
								&& programName.equalsIgnoreCase(CCIConstants.HSP_STP_IHP)
								&& participant.getSeason().getSeasonIhpdetails() != null
								&& !participant.getSeason().getSeasonIhpdetails().isEmpty()) {
							details.setParticipantSeasonName(
									participant.getSeason().getSeasonIhpdetails().get(0).getProgramName());
						}
					} catch (Exception e) {
						ExceptionUtil.logException(e, logger);
					}
				...
				addedParticipants.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO,
						ErrorCode.DEFAULT_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_SUCCESS)));
			} else {
				addedParticipants.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO,
						ErrorCode.DEFAULT_CODE.getValue(), messageUtil.getMessage(CCIConstants.NO_RECORD)));
			}
		} catch (Exception e) {
			addedParticipants.setStatus(componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR,
					ErrorCode.DEFAULT_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_FAILURE)));
			ExceptionUtil.logException(e, logger);
		}
		...
		try {
			...
			if (allPartners != null) {
				for (Partner p : allPartners) {
					// Bug : 1409, get only active sub-partners. crazy code but
					// no other option
					if (p.getPartnerReviewStatuses() != null) {
						if (p.getPartnerReviewStatuses().get(0) != null) {
							if (p.getPartnerReviewStatuses().get(0).getPartnerStatus2()
									.getPartnerStatusId() != CCIConstants.DELETED_STATUS) {
			...
			subPartners.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO,
					ErrorCode.DEFAULT_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_SUCCESS)));
		} catch (Exception e) {
			subPartners.setStatus(componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR,
					ErrorCode.DEFAULT_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_FAILURE)));
			ExceptionUtil.logException(e, logger);
		}
		return subPartners;
	...
	public WSDefaultResponse assignSubpartnerToParticipant(String subpartnerId, String participantId) {
		...
		try {
			...
			participantRepository.saveAndFlush(p);
			wsDefaultResponse.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO,
					ErrorCode.CHANGE_PARTICIPANT_SUBPARTNER.getValue(),
					messageUtil.getMessage(CCIConstants.SERVICE_SUCCESS)));
		} catch (Exception e) {
			ExceptionUtil.logException(e, logger);
			wsDefaultResponse.setStatus(componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR,
					ErrorCode.CANT_CHANGE_PARTICIPANT_SUBPARTNER.getValue(),
	...
	public WSDefaultResponse assignEmailToParticipant(String participantId, String email2) {
		...
		try {
			if (email2 != null) {
				...
				if (exists != null) {
					wsDefaultResponse.setStatus(componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR,
							ErrorCode.CHANGE_PARTICIPANT_EMAIL.getValue(),
					...
					if (participantId != null) {
						...
						wsDefaultResponse.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS,
								CCIConstants.TYPE_INFO, ErrorCode.CHANGE_PARTICIPANT_EMAIL.getValue(),
								messageUtil.getMessage(CCIConstants.SERVICE_SUCCESS)));
					} else {
						wsDefaultResponse.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS,
								CCIConstants.TYPE_INFO, ErrorCode.CHANGE_PARTICIPANT_EMAIL.getValue(),
			...
			ExceptionUtil.logException(e, logger);
			wsDefaultResponse.setStatus(componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR,
					ErrorCode.CANT_CHANGE_PARTICIPANT_EMAIL.getValue(),
	...
	public WSDefaultResponse changeParticipantStatus(String participantId, String status) {
		...
		try {
			...
			wsDefaultResponse.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO,
					ErrorCode.CHANGE_PARTICIPANT_STATUS.getValue(),
					messageUtil.getMessage(CCIConstants.SERVICE_SUCCESS)));
		} catch (Exception e) {
			ExceptionUtil.logException(e, logger);
			wsDefaultResponse.setStatus(componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR,
					ErrorCode.CANT_CHANGE_PARTICIPANT_STATUS.getValue(),
	...
	public Response resetParticipantPassword(String participantGoId, HttpServletRequest request) {
		...
		if (participantGoId == null) {
			response.setStatus(componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR,
					ErrorCode.ERROR_GET_PARTNER_SEASON.getValue(), "invalid participant id"));
				...
				if (paricipant != null) {
					...
					if (participantLogin != null) {
						String body = "<p>Ciao! </p>"
								...
								+ "<p>Please click on the link below to create a new password:</p> " + "<p>"
								+ formResetURL(request).concat(participantLogin.getKeyValue()) + "</p>"
						...
						response.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO,
								ErrorCode.UTILITY_SERVICE_CODE.getValue(),
								"An email has been sent to address " + "\'" + participantLogin.getEmail() + "\'"
										+ " for login name " + "\'" + participantLogin.getLoginName() + "\'"
										+ " with instructions to reset password"));
					} else {
						response.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO,
								ErrorCode.NO_RECORD.getValue(), messageUtil.getMessage(CCIConstants.NO_RECORD)));
						logger.error(messageUtil.getMessage(CCIConstants.NO_RECORD));
					}
				}
			} catch (CcighgoException e) {
				response.setStatus(componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR,
						ErrorCode.ERROR_GET_PARTNER_SEASON.getValue(), "an error occurred while reseting password"));
				logger.error(
	...
	public Response sendLogin(String participantGoId, HttpServletRequest request) {
		Response response = new Response();
		try {
			if (participantGoId == null || Integer.valueOf(participantGoId) == 0
					|| Integer.valueOf(participantGoId) < 0) {
			...
			if (participantLoginData != null) {
				String body = "<p>Ciao! </p>"
						+ "<p>This email was sent automatically by Greenheart Online (GO) in response to your request for a new password. </p>"
						+ "<p>" + "Your username is : " + participantLoginData.getLoginName() + "</p>"
						+ "<p>Please click on the link below to create a new password:</p> " + "<p>"
						+ formResetURL(request).concat(participantLoginData.getKeyValue()) + "</p>"
						+ "<p>If you didn't request a new password, please let us know.</p>" + "<p>Thank you,</p>"
						+ "<p>CCI Greenheart.</p>";
				email.send(participantLoginData.getEmail(), CCIConstants.RESET_PASSWORD_SUBJECT, body, true);
				response.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO,
						ErrorCode.UTILITY_SERVICE_CODE.getValue(),
						"An email has been sent to address " + "\'" + participantLoginData.getEmail() + "\'"
								+ " for login name " + "\'" + participantLoginData.getLoginName() + "\'"
								+ " with instructions to reset password"));
			} else {
				response.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO,
						ErrorCode.NO_RECORD.getValue(), messageUtil.getMessage(CCIConstants.NO_RECORD)));
			...
			response.setStatus(componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR,
					ErrorCode.ERROR_GET_PARTNER_SEASON.getValue(), e.getMessage()));
	...
	public SeasonsForParticipants getAllAvailableSeasons2(String partnerId, String participantId) {
		...
		try {
			...
			List<Object[]> result = em.createNativeQuery("call SPPartnerParticipantSeasons(:partnerId,:participantId)")
					.setParameter("partnerId", partnerId).setParameter("participantId", participantId).getResultList();
			if (result != null) {
