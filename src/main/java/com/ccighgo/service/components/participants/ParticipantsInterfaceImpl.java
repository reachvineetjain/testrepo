...
//						participant.setEmail(p.getEmail());
						...
						if (season != null && partner != null) {
							PartnerSeason ps = partnerSeasonsRepository.findPartnerSeasonBySeasonIdProgramIdPartnerGoId(season.getSeasonId(), p.getDepartmentId(), partner.getGoIdSequence().getGoId());
							if (ps != null) {
	...
	public NewManualParticipant editNewParticipant(Integer participantId) {
		...
		try {
			...
			editedParticipant
					.setStatus(componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR, ErrorCode.DEFAULT_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_FAILURE)));
			ExceptionUtil.logException(e, logger);
		}
	...
	public NewManualParticipant updateParticipant(NewManualParticipant participant) {
		try {
			...
			participant.setStatus(componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR, ErrorCode.DEFAULT_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_FAILURE)));
			ExceptionUtil.logException(e, logger);
		}
		return participant;
	}
...
//					details.setParticipantEmail(participant.getEmail());
	...
	public SubPartnersForParticipants getAllAvailableSubPartners(int partnerId) {
		...
		try {
			...
			subPartners.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO, ErrorCode.DEFAULT_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_SUCCESS)));
		} catch (Exception e) {
			subPartners.setStatus(componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR, ErrorCode.DEFAULT_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_FAILURE)));
			ExceptionUtil.logException(e, logger);
		}
		return subPartners;
	}
				...
				if (paricipant != null) {
					...
					if (participantLogin != null) {
						String body = "<p>Ciao! </p>" + "<p>This email was sent automatically by Greenheart Online (GO) in response to your request for a new password. </p>" + "<p>"
								...
								+ formResetURL(request).concat(participantLogin.getKeyValue()) + "</p>" + "<p>If you didn't request a new password, please let us know.</p>" + "<p>Thank you,</p>"
								+ "<p>CCI Greenheart.</p>";
						...
						response.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO, ErrorCode.UTILITY_SERVICE_CODE.getValue(), "An email has been sent to address "
								+ "\'" + participantLogin.getEmail() + "\'" + " for login name " + "\'" + participantLogin.getLoginName() + "\'" + " with instructions to reset password"));
					} else {
	...
	public Response sendLogin(String participantGoId, HttpServletRequest request) {
		...
		try {
			...
			if (participantLoginData != null) {
				String body = "<p>Ciao! </p>" + "<p>This email was sent automatically by Greenheart Online (GO) in response to your request for a new password. </p>" + "<p>" + "Your username is : "
						+ participantLoginData.getLoginName() + "</p>" + "<p>Please click on the link below to create a new password:</p> " + "<p>"
						+ formResetURL(request).concat(participantLoginData.getKeyValue()) + "</p>" + "<p>If you didn't request a new password, please let us know.</p>" + "<p>Thank you,</p>"
						+ "<p>CCI Greenheart.</p>";
				email.send(participantLoginData.getEmail(), CCIConstants.RESET_PASSWORD_SUBJECT, body, true);
				response.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO, ErrorCode.UTILITY_SERVICE_CODE.getValue(), "An email has been sent to address " + "\'"
						+ participantLoginData.getEmail() + "\'" + " for login name " + "\'" + participantLoginData.getLoginName() + "\'" + " with instructions to reset password"));
			} else {
				response.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO, ErrorCode.NO_RECORD.getValue(), messageUtil.getMessage(CCIConstants.NO_RECORD)));
				logger.error(messageUtil.getMessage(CCIConstants.NO_RECORD));
			}
