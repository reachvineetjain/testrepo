...
public class UpdateEVProgramEditSubjectBatchDataService implements IEVBatchDataService {
   ...
   public SEVISBatchCreateUpdateEV fetchBatchData(BatchParam batchParam) {
      ...
      // get EVs from DB
      // @formatter:off
      List<Integer> participantIds = batchParam.getParticipant().stream().map(p -> p.getParticipantGoId()).collect(toList());
      // @formatter:on

      List<Participant> participants = participantRepository.findByParticipantGoIdIn(participantIds);

      // @formatter:off
      List<ExchangeVisitor> evs = participants.stream().map(p -> intoEV(p, batchParam.getUserId(), "N0000000000", "1")).collect(toList());
      // @formatter:on
