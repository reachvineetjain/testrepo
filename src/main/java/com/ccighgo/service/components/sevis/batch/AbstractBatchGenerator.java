...
public abstract class AbstractBatchGenerator<T> implements IBatchGenerator<T> {
   ...
   @Override
   public BatchDetails createBatch(BatchParam batchParam, IBatchDataService<T> dataService, ServletContext servletContext) {
      /*
       * Template Method
       */
   ...
   protected Status createFailureStatus() {
      Status status = componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR, SevisBatchErrorCode.BATCH_CREATE_FAILED.getValue(), "Batch Creation Failed.");
      return status;
   }

   protected Status createFailureStatus(String msg) {
      Status status = componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR, SevisBatchErrorCode.BATCH_CREATE_FAILED.getValue(), msg);
      return status;
   }

   protected Status createSuccessStatus() {
      Status status = componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO, SevisBatchErrorCode.POST_PARAM_NA.getValue(),
   ...
   protected BatchDetails createInvalidArgsResponse() {
      ...
      Status status = componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_ERROR, SevisBatchErrorCode.POST_PARAM_NA.getValue(), "Invalid Arguments.");
