...
public class FieldStaffImpl implements FieldStaffsInterface {
	...
	public Response updateFieldStaffDetail(FieldStaffDetails fieldStaffDetail) {
		...
		try {
         ...
         fs.setTotalPlacementsManual(fieldStaffDetail.getTotalPlacementManual());       
			...
			response.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO, ErrorCode.FIELDSTAFF_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_SUCCESS)));
		} catch (Exception e) {
			ExceptionUtil.logException(e, LOGGER);
			response.setStatus(componentUtils.getStatus(CCIConstants.FAILURE, CCIConstants.TYPE_INFO, ErrorCode.FIELDSTAFF_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_FAILURE)));
		...
		try {
			...
			if (login != null) {
				String body = "<p>Ciao! </p>" + "<p>This email was sent automatically by Greenheart Online (GO) in response to your request for a new password. </p>" + "<p>" + "Your username is : "
						+ login.getLoginName() + "</p>" + "<p>Please click on the link below to create a new password:</p> " + "<p>" + formResetURL(request).concat(login.getKeyValue()) + "</p>"
	...
	public AdminFieldStaffHostFamily getFSHostFamilies(int fieldStaffId, int flagId, String category) {
		...
		try {
			...
			try {
				result = em.createNativeQuery("call SPFieldStaffHostFamilyList(:fieldStaffId,:flagId,:category)").setParameter("fieldStaffId", fieldStaffId).setParameter("flagId", flagId)
			...
			if (result != null) {
				if (!result.isEmpty()) {
					...
					pwqa.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO, ErrorCode.FS_ADMIN_HOST_FAMILY.getValue(), messageUtil.getMessage(CCIConstants.NO_RECORD)));
				}
			} else {
				pwqa.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS, CCIConstants.TYPE_INFO, ErrorCode.EMPTY_FS_ADMIN_HOST_FAMILY.getValue(), messageUtil.getMessage(CCIConstants.NO_RECORD)));
			}
		} catch (Exception e) {
			ExceptionUtil.logException(e, LOGGER);
	...
	public Response updateFieldStaffStatus(String fsgoId, String loginId, String statusId) {
		...
		try {
			...
			if (fieldStaff == null) {
				response.setStatus(componentUtils.getStatus(CCIConstants.NO_RECORD, CCIConstants.TYPE_INFO, ErrorCode.FIELDSTAFF_CODE.getValue(), messageUtil.getMessage(CCIConstants.SERVICE_SUCCESS)));
			}
