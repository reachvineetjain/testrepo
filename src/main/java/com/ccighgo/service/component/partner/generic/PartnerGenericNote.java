...
public class PartnerGenericNote implements PartnerGenericNoteInterface {
	...
	public WSDefaultResponse deleteNote(DeleteNote deleteNote) {
		...
		try {
			PartnerNote noteEntity = partnerNoteRepository.findOne(deleteNote
					.getNoteId());

			partnerNoteRepository.delete(noteEntity);

			responce.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS,
					CCIConstants.TYPE_INFO, ErrorCode.NOTE_DELETED.getValue(),
					messageUtil.getMessage(CCIConstants.SERVICE_SUCCESS)));
			...
			responce.setStatus(componentUtils.getStatus(
					...
					ErrorCode.FAILED_TO_CREATE_NOTE.getValue(),
					messageUtil
							.getMessage(GenericMessageConstants.FAILED_TO_VIEW_GENERIC_NOTE)));
			LOGGER.error(messageUtil
					.getMessage(GenericMessageConstants.FAILED_TO_VIEW_GENERIC_NOTE));
		}
		return responce;
	...
	public Topics viewTopics(int partnerId) {
		...
		try {
			...
			if (partnerTopics == null) {
				topicsList
						.setStatus(componentUtils.getStatus(
								CCIConstants.FAILURE,
								CCIConstants.TYPE_ERROR,
								ErrorCode.FAILED_TO_VIEW_NOTE.getValue(),
								messageUtil
			...
			topicsList.setStatus(componentUtils.getStatus(CCIConstants.SUCCESS,
					CCIConstants.TYPE_INFO, ErrorCode.NOTE_VIEW.getValue(),
					messageUtil.getMessage(CCIConstants.SERVICE_SUCCESS)));
		} catch (Exception e) {
			ExceptionUtil.logException(e, LOGGER);
			topicsList
