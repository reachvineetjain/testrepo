...
public interface CCISaffDefaultPermissionRepository extends JpaRepository<CCIStaffRolesDefaultResourcePermission, Integer> {
   ...
   /*
    * @Query( value =
    * "SELECT dg.`departmentResourceGroupId`,dg.`resourceGroupName`,rp.`resourcePermissionId` "
    * +
    ...
    * + " WHERE cr.`cciStaffRoleId`= ?1 AND dg.`departmentId`=?2 ", nativeQuery
    * = true) List<Object []> getDefaultPermissions(Integer id,Integer deptId);
    */
