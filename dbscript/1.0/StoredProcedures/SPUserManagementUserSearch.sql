...
CREATE DEFINER=root@localhost PROCEDURE SPUserManagementUserSearch(IN CCIId INT(11),IN  FName VARCHAR(50), IN LName VARCHAR(50), IN LogName VARCHAR(50),IN CountryId INT(5),IN Mail VARCHAR(100), IN Rolelist VARCHAR(50),IN DepList VARCHAR(50), IN PgmList VARCHAR(50),IN Active VARCHAR(10))
BEGIN
	...
	SET @stmt = CONCAT ('SELECT cci.cciStaffUserID
		...
		INNER JOIN CCIStaffUserProgram csp ON csp.cciStaffUserId = cci.cciStaffUserId',@wher,' GROUP BY cci.cciStaffUserId');                              
